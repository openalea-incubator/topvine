...
def topiary(shoot, allo, lawf, omega = 45*numpy.pi/180., LongPetiole = 0.12, visu_en = True, num_vine=1, num_shoot=1):
        ...
        for phyto in range(NombrePhyto) :
            ...
            coordF = set_coordF(coord, numpy.array([Xi,Yi,Zi]), shoot.geom[2] *numpy.pi/180, (-90+CourbureActu)*numpy.pi/180)
            ...
            if len(shoot.topo[phyto])>1:
                Lb = allo_LN(allo[1], len(shoot.topo[phyto])-1) #en metres
