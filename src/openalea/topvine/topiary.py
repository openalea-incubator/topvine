...
class Topiary:
    ...
    def __init__(
            ...
            num_vine: int = 1,
            num_shoot: int = 1,
            is_use_legacy_labelling: bool = False,
    ):
        ...
        for phyto in range(NombrePhyto):
            ...
            Xii = numpy.cos(CourbureActu * numpy.pi / 180) * Lin * numpy.cos(shoot.geom[2] * numpy.pi / 180) + Xi
            Yii = numpy.cos(CourbureActu * numpy.pi / 180) * Lin * numpy.sin(shoot.geom[2] * numpy.pi / 180) + Yi
            Zii = numpy.sin(CourbureActu * numpy.pi / 180) * Lin + Zi

            # mise en place de l'entre noeud I
            if visu_en == True:
                en = en0()
                t_en = transformation(en, 0.01, 0.01, Lin, shoot.geom[2] * numpy.pi / 180,
                                      (90 - CourbureActu) * numpy.pi / 180, 0, Xi, Yi, Zi)
                t_ens = Shape(t_en, Material(Color3(198, 179, 37)))
                t_ens.setName(self.vine_label(0, (phyto + 1), num_shoot, num_vine, is_use_legacy_labelling))
                scene.add(t_ens)

            # mise en place de la feuille I
            Lb = LongPetiole  # to make it depend on lin
            coord = shoot.topo[phyto][0].set_coord0(Lin, Lb, omega)
            coordF = self.set_coordF(coord, numpy.array([Xi, Yi, Zi]), shoot.geom[2] * numpy.pi / 180,
                                     (-90 + CourbureActu) * numpy.pi / 180)

            NSstat = self.NSstatus(coordF, LimNS)
            angles = shoot.topo[phyto][0].set_anglesF(lawf, NSstat)
            tir1 = angles[
                       1] * numpy.pi / 180  # Tirage_anglesF (Tab_loi_feuilles[2][0], Tab_loi_feuilles[2][1], Tab_loi_feuilles[2][2])*numpy.pi/180
            tir2 = -(90 - angles[
            ...
            fIs.setName(self.vine_label(1, int(shoot.topo[phyto][0].id), num_shoot,
                                        num_vine, is_use_legacy_labelling))  # stockage d'un label canestra dans l'objet geom
            scene.add(fIs)
            ...
            if len(shoot.topo[phyto]) > 1:
                ...
                for i in range(1, len(shoot.topo[phyto])):
                    ...
                    fIIs.setName(self.vine_label(1, int(shoot.topo[phyto][i].id), num_shoot, num_vine, is_use_legacy_labelling))
                    scene.add(fIIs)
    ...
    def vine_label(self, sp_opt, num_phy, num_ram, num_vine, is_legacy: bool):
        if is_legacy:
            lab = str(int(sp_opt * 10 ** 11 + num_phy * 10 ** 6 + num_ram * 10 ** 3 + num_vine))
            res = (12 - len(lab)) * '0' + lab
        else:
            res = str(str(sp_opt) + "type" + str(num_phy) + "phy" + str(num_ram) + "ram" + str(num_vine) + "plant")
        return res
